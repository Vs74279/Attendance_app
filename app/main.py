...
@app.middleware("http")
async def add_user_id_to_cookies(request: Request, call_next):
    response = await call_next(request)
    if request.cookies.get("user_id"):
        response.set_cookie(key="user_id", value=request.cookies.get("user_id"))
    return response
...
async def get_monthly_attendance(request: Request, db: Session = Depends(get_db)):
    ...
    # Add default status for dates without records
...
async def get_all_users_attendance(request: Request, db: Session = Depends(get_db)):
    user_id = request.cookies.get("user_id")
    if not user_id:
        raise HTTPException(status_code=401, detail="Not authenticated")
